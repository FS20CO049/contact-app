...
const Sidebar: React.FC<SidebarProps> = ({ showSideBar, setShowSideBar }) => {
  return (
    <div
      className={`bg-slate-200 text-black h-full sm:w-1/5 w-1/2 pt-6 fixed top-0 left-0 ${
        showSideBar ? 'translate-x-0 z-50' : '-translate-x-full z-0'
      } transition-transform duration-300 ease-in-out`}
    >
      <div className="flex justify-between items-center px-5">
        <h2 className="text-cyan-500 font-bold text-2xl my-3">CovidData</h2>
        <button
      ...
      <ul className="space-y-2">
        <Link to={'/'}>
          <li className="hover:bg-cyan-200 px-2 py-2 sm:px-10 sm:py-5">
            Contacts
          </li>
        </Link>
        <Link to={'/covid-stats'}>
